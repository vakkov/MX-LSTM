...
def getGridMask(frame, dimensions, neighborhood_size, grid_size):
    ...
    for pedindex in range(mnp):
        ...
        vis_x, vis_y = frame[pedindex, 3], frame[pedindex, 4]
        tv1=[vis_x-current_x,vis_y-current_y]
        v1=np.array(tv1)
        ...
        # editing irtiza start here


        # editing irtiza end here
        if vis_x >= width_high or vis_x < width_low or vis_y >= height_high or vis_y < height_low:
            # Ped not in surrounding, so binary mask should be zero
            continue

        cell_x_vis = int(np.floor(((vis_x - width_low) / width_bound) * grid_size))
        cell_y_vis = int(np.floor(((vis_y - height_low) / height_bound) * grid_size))
        ...
        for otherpedindex in range(mnp):
            ...
            tv2 = [other_x - current_x, other_y - current_y]
            v2=np.array(tv2)
            numTerm=v1.dot(v2)
            denTerm=np.linalg.norm(tv1)*np.linalg.norm(tv2)
            thetaRad=np.arccos((numTerm/denTerm))
            thetaDeg=mth.degrees(thetaRad)


            if thetaDeg<=30:
            # Other ped is in the corresponding grid cell of current ped only weight if view frustum is in that grid
